...
Процедура ParentCompanies(Настройки) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокиФактАдрес = КонтактнаяИнформация.НайтиСтроки(Новый Структура("Ссылка,Вид", Выборка.Ссылка, Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента));
		...
		КонецЕсли;
		Если СтрокиФактАдрес.Количество() > 0 Тогда
			ФактАдрес = Лев(СтрокиФактАдрес[0].Представление, 254);
		КонецЕсли;
...
Процедура Outlets(Настройки) Экспорт
	...
	Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
		...
		// TODO: Оптимизировать получение контаргентов и их кодов
		КонтрагентПартнера = ПартнерыИКонтрагенты.ПолучитьКонтрагентаПартнераПоУмолчанию(Выборка.Ссылка);
		ВнешнийКодКонтрагента = Формат(УправлениеСвойствами.ЗначениеСвойства(КонтрагентПартнера, СвойствоКонтрагенты), "ЧГ=;");
		...
		ЗаписатьРеквизитXML(ЗаписьXML, "DC_ALLOW",		"");
...
Процедура SalOuts(Настройки)
	...
	Запрос.Текст =
		...
		|	МАКСИМУМ(ТекущаяДата),
		|	МАКСИМУМ(Статус),
		|	МАКСИМУМ(ЦенаВключаетНДС) КАК ЦенаВключаетНДС
		|ПО
