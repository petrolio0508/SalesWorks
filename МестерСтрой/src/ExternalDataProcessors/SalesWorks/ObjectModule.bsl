...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегестрации.Вид =ДополнительныеОтчетыИОбработкиКлиентСервер.ВидОбработкиДополнительнаяОбработка();
	ПараметрыРегестрации.Наименование = "Обмен с хенкель";
	ПараметрыРегестрации.БезопасныйРежим = Ложь; 
	
	НоваяКоменды = ПараметрыРегестрации.Команды.Добавить();
	НоваяКоменды.Представление = "Полная выгрузка";
	НоваяКоменды.Идентификатор = "ХЕНКЕЛЬ_ПОЛНАЯ_ВЫГРУЗКА";
	НоваяКоменды.Использование = ДополнительныеОтчетыИОбработкиКлиентСервер.ТипКомандыВызовСерверногоМетода();	
	
	НоваяКоменды = ПараметрыРегестрации.Команды.Добавить();
	НоваяКоменды.Представление = "Частичная выгрузка";
	НоваяКоменды.Идентификатор = "ХЕНКЕЛЬ_ЧАСТИЧНАЯ_ВЫГРУЗКА";
	НоваяКоменды.Использование = ДополнительныеОтчетыИОбработкиКлиентСервер.ТипКомандыВызовСерверногоМетода();
...
Процедура ВыполнитьКоманду(ИдентификаторКоманды) Экспорт
	...
	Если ИдентификаторКоманды = "ХЕНКЕЛЬ_ПОЛНАЯ_ВЫГРУЗКА" Тогда
		ЧислоДня = День(ТекущаяДатаСеанса());
		Если ЧислоДня <=3 Тогда
			ДатаНачала = НачалоМесяца(ДобавитьМесяц(ТекущаяДатаСеанса(), - 1));
	...
	Если ИдентификаторКоманды = "ХЕНКЕЛЬ_ЧАСТИЧНАЯ_ВЫГРУЗКА" Тогда
		ДатаНачала = НачалоДня(ТекущаяДатаСеанса());
		ДатаОкончания = КонецДня(ТекущаяДатаСеанса());	
		ВыгрузитьВсеДанные(ДатаНачала, ДатаОкончания, Истина);
	КонецЕсли;

КонецПроцедуры

Процедура ВыгрузитьВсеДанные(ДатаНачала = Неопределено, ДатаОкончания = Неопределено, ЭтоЧастичнаяВыгрузка = Ложь) Экспорт
	...
	Настройки.Вставить("ЭтоЧастичнаяВыгрузка", ЭтоЧастичнаяВыгрузка);
	...
	Если Не ЭтоЧастичнаяВыгрузка Тогда
		PriceList_v2(Настройки);
...
Процедура SalIns(Настройки)
	
	ИмяФайла = Настройки.КаталогВыгрузки + "SalIns.xml";
	
	ДатаНачала = Настройки.ДатаНачала;
	ДатаОкончания = Настройки.ДатаОкончания;
	
	СвойствоПартнер = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя",
		"Партнеры_SalesWorks_CODE");
	СвойствоСклад = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя",
		"Склады_SalesWorks_CODE");
	СвойствоНоменклатура = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя",
		"Номенклатура_SalesWorks_CODE");
	...
	МассивХозяйственныхОпераций.Добавить(Перечисления.ХозяйственныеОперации.ВозвратТоваровПоставщику);
	...
	СоответсвиеТиповДокументов.Вставить(Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика, "1");
	СоответсвиеТиповДокументов.Вставить(Перечисления.ХозяйственныеОперации.ВозвратТоваровПоставщику, "4");
	...
	|	"""" КАК ЗаказКлиентаНомер,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ЗаказКлиентаДата
	|ИЗ
	|	Документ.ПриобретениеТоваровУслуг.Товары КАК ВыручкаИСебестоимостьПродажОбороты
	...
	|	И ВыручкаИСебестоимостьПродажОбороты.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ДАТАВРЕМЯ(1, 1, 1)
	|ИЗ
	|	Документ.ВозвратТоваровПоставщику.Товары КАК ВозвратТоваровОтКлиентаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДополнительныеСведения КАК ДополнительныеСведения
	...
	|	И ВозвратТоваровОтКлиентаТовары.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	|ИТОГИ
	...
	ЗаписьXML.ЗаписатьНачалоЭлемента("SalIns"); // SalOuts
	Пока ВыборкаХозяйственнаяОперация.Следующий() Цикл
		...
		Пока ВыборкаДокументПродажи.Следующий() Цикл
			...
			ЗаписьXML.ЗаписатьНачалоЭлемента("SalIn"); // SalIn

			НомерДокумента = СтрШаблон(ШаблонНомераДокумента, СоответсвиеТиповДокументов.Получить(
				ВыборкаХозяйственнаяОперация.ХозяйственнаяОперация), Формат(ВыборкаДокументПродажи.ДокументПродажиДата,
				"ДФ=yyyy;"), ВыборкаДокументПродажи.ДокументПродажиНомер);
			...
			ЗаписатьРеквизитXML(ЗаписьXML, "DOC_TYPE", СоответсвиеТиповДокументов.Получить(
				ВыборкаХозяйственнаяОперация.ХозяйственнаяОперация));
			ЗаписатьРеквизитXML(ЗаписьXML, "CUSTDOC_NO", ""); // TODO: Не понятное поле
			...
			ЗаписьXML.ЗаписатьНачалоЭлемента("SalInLocalDetails");
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				
				ЗаписьXML.ЗаписатьНачалоЭлемента("SalInLocalDetail");
				
				ЗаписатьРеквизитXML(ЗаписьXML, "LOCALCODE", Формат(ВыборкаДетальныеЗаписи.НоменклатураВнешнийКод,
					"ЧГ="));
				...
				ЗаписатьРеквизитXML(ЗаписьXML, "DTLM", Формат(ВыборкаДетальныеЗаписи.ТекущаяДата,
					"ДФ='yyyyMMdd HH:mm'"));
				ЗаписатьРеквизитXML(ЗаписьXML, "QTY", Формат(ВыборкаДетальныеЗаписи.Количество,
					"ЧЦ=14; ЧДЦ=3; ЧРД=.; ЧГ=;"));	
				ЗаписатьРеквизитXML(ЗаписьXML, "PRICE", Формат(ВыборкаДетальныеЗаписи.Сумма
					/ ВыборкаДетальныеЗаписи.Количество, "ЧЦ=15; ЧДЦ=8; ЧРД=.; ЧН=0; ЧГ=;"));		
...
Процедура Discounts(Настройки)

	ИмяФайла = Настройки.КаталогВыгрузки + "Discounts.xml";
	...
	ЗаписьXML.ЗаписатьНачалоЭлемента("ROOT"); // ROOT
	ЗаписьXML.ЗаписатьНачалоЭлемента("OutletDiscounts"); // OutletPayForms
...
Процедура Denials(Настройки)

	ИмяФайла = Настройки.КаталогВыгрузки + "Denials.xml";
	...
	ЗаписьXML.ЗаписатьНачалоЭлемента("ROOT"); // ROOT
	ЗаписьXML.ЗаписатьНачалоЭлемента("OutletOrderDenials"); // OutletPayForms
...
Процедура OutletPayForms(Настройки)
	...
	СвойствоПартнер = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя",
		"Партнеры_SalesWorks_CODE");
	СвойствоВидЦены = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя",
		"ВидыЦен_SalesWorks_CODE");
	...
	Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
		...
		ЗаписатьРеквизитXML(ЗаписьXML, "PAYF_CODE", "100"); //TODO: Код вида цены
...
Процедура ParentCompanies(Настройки) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокиЮрАдрес = КонтактнаяИнформация.НайтиСтроки(Новый Структура("Ссылка,Вид", Выборка.Ссылка,
			Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента));
		СтрокиТелефона = КонтактнаяИнформация.НайтиСтроки(Новый Структура("Ссылка,Вид", Выборка.Ссылка,
			Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента));
		СтрокиЭлектроннаяПочта = КонтактнаяИнформация.НайтиСтроки(Новый Структура("Ссылка,Вид", Выборка.Ссылка,
		...
		Если СтрокиТелефона.Количество() > 0 Тогда
			Если ЗначениеЗаполнено(СтрокиТелефона[0].НомерТелефона) И СтрДлина(СтрокиТелефона[0].НомерТелефона) <= 20 Тогда
				Телефон = ЛЕВ(СтрокиТелефона[0].НомерТелефона, 20); //TODO: уберите из заполнения атрибута PC_PHONE все символы и буквы, не относящиеся к номеру телефона
			КонецЕсли;
		КонецЕсли;
...
Процедура Outlets(Настройки) Экспорт
	...
	Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
		...
		Если Не КонтрагентПартнера.Пустая() Тогда
			...
			//@skip-check property-return-type
			ВнешнийКодКонтрагента = Настройки.СлужебныеДанные.ТаблицаКонтрагенты.Найти(КонтрагентПартнера,
				"Контрагент").ВнешнийКодКонтрагент;
		КонецЕсли;

		ЗаписьXML.ЗаписатьНачалоЭлемента("Outlet"); //Outlet

		СтрокиАдрес = КонтактнаяИнформация.НайтиСтроки(Новый Структура("Ссылка,Вид", Выборка.Ссылка,
			Справочники.ВидыКонтактнойИнформации.АдресПартнера));
		СтрокиАдресДоставки = КонтактнаяИнформация.НайтиСтроки(Новый Структура("Ссылка,Вид", Выборка.Ссылка,
			Справочники.ВидыКонтактнойИнформации.АдресПартнера));
		СтрокиТелефона = КонтактнаяИнформация.НайтиСтроки(Новый Структура("Ссылка,Вид", Выборка.Ссылка,
		...
		КонецЕсли;
		Если СтрокиТелефона.Количество() > 0 Тогда
			Если ЗначениеЗаполнено(СтрокиТелефона[0].НомерТелефона) Тогда
				Телефон = Лев(СтрокиТелефона[0].НомерТелефона, 20); //TODO: уберите из заполнения атрибута PC_PHONE все символы и буквы, не относящиеся к номеру телефона
			КонецЕсли;
		КонецЕсли;
		Если СтрокиЭлектроннаяПочта.Количество() > 0 Тогда
...
Процедура LocalProducts(Настройки) Экспорт
	...
	Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
		...
		ВнешнийКодНоменклатура = Формат(Выборка.ВнешнийКод, "ЧГ=");

		ЗаписатьРеквизитXML(ЗаписьXML, "LOCALCODE", ВнешнийКодНоменклатура);
		...
		ЗаписатьРеквизитXML(ЗаписьXML, "PACK_QTY", Формат(?(ЗначениеЗаполнено(Выборка.КоличествоВУпаковке),
			Выборка.КоличествоВУпаковке, 1), "ЧРД=.; ЧН=0; ЧГ=;"));
		...
		НоваяСтрокаСлужебныеДанные = Настройки.СлужебныеДанные.ТаблицаНоменклатура.Добавить();
		//@skip-check property-return-type
		НоваяСтрокаСлужебныеДанные.ВнешнийКодНоменклатура = ВнешнийКодНоменклатура;
		//@skip-check property-return-type
		НоваяСтрокаСлужебныеДанные.Номенклатура = Выборка.Ссылка;
...
Процедура PriceList_v2(Настройки) Экспорт
	...
	ПараметрыЗаполнения = Новый Структура;
		...
		ПараметрыЗаполнения.Вставить("Соглашение", Настройки.ВидЦены);
		ПараметрыЗаполнения.Вставить("РассчитыватьНаборы", Ложь);
		ПараметрыЗаполнения.Вставить("НалогообложениеНДС", Перечисления.ТипыНалогообложенияНДС.ПустаяСсылка());
		ПараметрыЗаполнения.Вставить("ВозвращатьМногооборотнуюТару", Ложь);
		ПараметрыЗаполнения.Вставить("ПоляЗаполнения", "Цена");
			
	ТаблицаЗначенийВсеТоварыХенкель = ТаблицаЗначенийВсеТоварыХенкель(Настройки);
	ТаблицаЗначенийВсеТоварыХенкель.Колонки.Добавить("Цена", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
	ТаблицаЗначенийВсеТоварыХенкель.Колонки.Добавить("Характеристика", Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
		Тип("СправочникСсылка.ХарактеристикиНоменклатуры"))));
	ТаблицаЗначенийВсеТоварыХенкель.Колонки.Добавить("Упаковка", Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
		Тип("СправочникСсылка.УпаковкиЕдиницыИзмерения"))));
		
		ЦеныРассчитаны = ЦеныПредприятияЗаполнениеСервер.ЗаполнитьЦены(
			ТаблицаЗначенийВсеТоварыХенкель,
	...
	Для Каждого Выборка Из ТаблицаЗначенийВсеТоварыХенкель Цикл
		...
		ЗаписатьРеквизитXML(ЗаписьXML, "PAYFORM_ID", "0");
		ЗаписатьРеквизитXML(ЗаписьXML, "PAYF_CODE", "100");
		ЗаписатьРеквизитXML(ЗаписьXML, "PRICE", Формат(Выборка.Цена, "ЧРД=.; ЧГ="));
		ЗаписатьРеквизитXML(ЗаписьXML, "STATUS", "2");
		ЗаписатьРеквизитXML(ЗаписьXML, "DTLM", Формат(ТекущаяДатаСеанса(), "ДФ='yyyyMMdd HH:mm'"));
...
Процедура PriceList(Настройки) Экспорт
	...
	Возврат; // TODO: Не используется
	
	ИмяФайла = Настройки.КаталогВыгрузки + "PriceList.xml";

	СвойствоВидыЦен = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя",
		"ВидыЦен_SalesWorks_CODE");
	СвойствоНоменклатура = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя",
		"Номенклатура_SalesWorks_CODE");
	ЗаписьXML = Новый ЗаписьXML;
	...
	Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
		...
		ВидЦеныВнешнийКод = Формат(Выборка.ВидЦеныВнешнийКод, "ЧГ=");
		Если ВидЦеныВнешнийКод = "4" Тогда //TODO: Пропускаю этот вид. Пока что. Надо что бы все выгружаемые виды были заведены на портале
			Продолжить;
		КонецЕсли;
		...
		ЗаписатьРеквизитXML(ЗаписьXML, "PAYFORM_ID", "0");
		ЗаписатьРеквизитXML(ЗаписьXML, "PAYF_CODE", ВидЦеныВнешнийКод);
		ЗаписатьРеквизитXML(ЗаписьXML, "PRICE", Формат(Выборка.Цена, "ЧРД=.; ЧГ="));
		ЗаписатьРеквизитXML(ЗаписьXML, "STATUS", Выборка.Статус);
		ЗаписатьРеквизитXML(ЗаписьXML, "DTLM", Формат(Выборка.ТекущаяДата, "ДФ='yyyyMMdd HH:mm'"));
...
Процедура ArchivedStocks(Настройки) Экспорт
	...
	МассивДатОстатков = Новый Массив; // Массив из Дата	
	ДатаНачалаОстатка = Настройки.ДатаНачала;
	ДатаОкончанияОстатков = Настройки.ДатаОкончания;
			
	Если ЗначениеЗаполнено(ДатаНачалаОстатка) И ЗначениеЗаполнено(ДатаОкончанияОстатков) Тогда
		ДатаОкончанияОстатков = КонецДня(ДатаОкончанияОстатков) + 1;
		ТекущийДень = ДатаНачалаОстатка;
		Пока ТекущийДень <= ДатаОкончанияОстатков Цикл
			МассивДатОстатков.Добавить(ТекущийДень);
			ТекущийДень = ТекущийДень + 86400;
		КонецЦикла;
	Иначе
		МассивДатОстатков.Добавить(НачалоДня(ТекущаяДатаСеанса()));
	КонецЕсли; 
	
	СвойствоНоменклаутра = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя",
		"Номенклатура_SalesWorks_CODE");
	СвойствоСклад = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя",
		"Склады_SalesWorks_CODE");
	...
	Для Каждого ДатаСрезаОстатков Из МассивДатОстатков Цикл
		
		Запрос.УстановитьПараметр("ДатаСрезаОстатков", ДатаСрезаОстатков);
		РезультатЗапроса = Запрос.Выполнить();
...
Процедура OutletDebts(Настройки) Экспорт
	...
	Пока ВыборкаГруппировкаПартнер.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			НомерДокумента = СтрШаблон(ШаблонНомераДокумента, "2", Формат(Выборка.РасчетныйДокументДата, "ДФ=yyyy;"),
				Выборка.РасчетныйДокументНомер);
			...
			ЗаписатьРеквизитXML(ЗаписьXML, "COMMENT", "" + НомерДокумента + " от " + Формат(
				Выборка.РасчетныйДокументДата, "ДФ=dd.MM.yyyy;"));
			ЗаписатьРеквизитXML(ЗаписьXML, "D_OVERDUE", ?(ДолгПросрочен, Формат(Выборка.ДолгОстаток, "ЧРД=.; ЧГ="),
				"0"));
			ЗаписатьРеквизитXML(ЗаписьXML, "STATUS", Выборка.Статус);
			ЗаписатьРеквизитXML(ЗаписьXML, "DTLM", Формат(Выборка.ТекущаяДата, "ДФ='yyyyMMdd HH:mm'"));
...
Процедура Orders(Настройки)
	...
	КодыМенеджеровСоответсвие = ПолучитьКодыМенеджеров(Настройки);
	...
	|	ЗаказКлиентаТовары.СтавкаНДС.Ставка,
	...
	Пока ВыборкаСсылка.Следующий() Цикл
		...
		//TODO: Удалить из запроса ЗаказКлиентаТовары.Ссылка.Проведен после старта
		
		ЗаписьXML.ЗаписатьНачалоЭлемента("Order"); // Order

		ВнешнийКодПартнер = Настройки.СлужебныеДанные.ТаблицаПартнеры.Найти(ВыборкаСсылка.Партнер,
			"Партнер").ВнешнийКодПартнер;
		ВнешнийКодМенеджера = КодыМенеджеровСоответсвие.Получить(ВыборкаСсылка.Менеджер);
		Если Не ЗначениеЗаполнено(ВнешнийКодМенеджера) Тогда
			ВнешнийКодМенеджера = КодыМенеджеровСоответсвие.Получить(Настройки.ПользовательПоУмолчанию);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ВнешнийКодМенеджера) Тогда
			ВнешнийКодМенеджера = Настройки.ВиртуальныйТПКод;
		КонецЕсли;
		
		ДатаОтгрузки = ВыборкаСсылка.ДатаОтгрузки;
		Если НЕ ЗначениеЗаполнено(ДатаОтгрузки) Тогда
			ДатаОтгрузки = ВыборкаСсылка.Дата;
		КонецЕсли;

		ЗаписатьРеквизитXML(ЗаписьXML, "CORDER_NO", СформироватьНомерЗаказа(ВыборкаСсылка.Номер, ВыборкаСсылка.Дата));
		ЗаписатьРеквизитXML(ЗаписьXML, "OL_CODE", ВнешнийКодПартнер);
		ЗаписатьРеквизитXML(ЗаписьXML, "MERCH_CODE", ВнешнийКодМенеджера);
		...
		ЗаписатьРеквизитXML(ЗаписьXML, "DATETO", XMLСтрока(ДатаОтгрузки));
		...
		ЗаписьXML.ЗаписатьНачалоЭлемента("OrderLocalDetails");
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			ЗаписьXML.ЗаписатьНачалоЭлемента("OrderLocalDetail");

			ВнешнийКодНоменклатура = Настройки.СлужебныеДанные.ТаблицаНоменклатура.Найти(
				ВыборкаДетальныеЗаписи.Номенклатура, "Номенклатура").ВнешнийКодНоменклатура;
			...
			ЗаписатьРеквизитXML(ЗаписьXML, "QTY", Формат(ВыборкаДетальныеЗаписи.Количество, "ЧРД=.; ЧГ="));
			ЗаписатьРеквизитXML(ЗаписьXML, "VAT", XMLСтрока(?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.СтавкаНДС), ВыборкаДетальныеЗаписи.СтавкаНДС, "0")));
			ЗаписатьРеквизитXML(ЗаписьXML, "OLORDER_NO", "");	
...
Процедура SalOuts(Настройки)
	...
	СоответсвиеТиповДокументов.Вставить(Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию, "2");
	...
	|	ВыручкаИСебестоимостьПродажОбороты.Ссылка.Партнер КАК Партнер,
	|	ВыручкаИСебестоимостьПродажОбороты.Номенклатура КАК Номенклатура,
	|	СУММА(ВыручкаИСебестоимостьПродажОбороты.Количество) КАК Количество,
	|	СУММА(ВыручкаИСебестоимостьПродажОбороты.Сумма) КАК Сумма,
	|	ЕСТЬNULL(ДополнительныеСведения.Значение, 0) КАК ПартнерВнешнийКод,
	|	ДополнительныеСведения1.Значение КАК СкладВнешнийКод,
	|	ЕСТЬNULL(ДополнительныеСведения2.Значение, 0) КАК НоменклатураВнешнийКод,
	|	&ТекущаяДата КАК ТекущаяДата,
	...
	|	МАКСИМУМ(ВыручкаИСебестоимостьПродажОбороты.Ссылка.ЗаказКлиента.Менеджер) КАК Менеджер
	|ИЗ
	...
	|	ВыручкаИСебестоимостьПродажОбороты.Номенклатура.Производитель = &Производитель
	|	И ВыручкаИСебестоимостьПродажОбороты.Ссылка.ХозяйственнаяОперация В (&МассивХозяйственныхОпераций)
	|	И ВыручкаИСебестоимостьПродажОбороты.ЗаказКлиента <> ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)
	|	И ВыручкаИСебестоимостьПродажОбороты.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	|СГРУППИРОВАТЬ ПО
	|	ВыручкаИСебестоимостьПродажОбороты.Ссылка.ХозяйственнаяОперация,
	|	ВыручкаИСебестоимостьПродажОбороты.Ссылка,
	|	ВыручкаИСебестоимостьПродажОбороты.Ссылка.Номер,
	|	ВыручкаИСебестоимостьПродажОбороты.Ссылка.Дата,
	|	ВыручкаИСебестоимостьПродажОбороты.Ссылка.Партнер,
	|	ВыручкаИСебестоимостьПродажОбороты.Номенклатура,
	|	ЕСТЬNULL(ДополнительныеСведения.Значение, 0),
	|	ДополнительныеСведения1.Значение,
	|	ЕСТЬNULL(ДополнительныеСведения2.Значение, 0),
	|	ВЫБОР
	...
	|		ИНАЧЕ ""9""
	|	КОНЕЦ,
	|	ВыручкаИСебестоимостьПродажОбороты.СтавкаНДС.Ставка,
	|	ВЫБОР
	|		КОГДА ВыручкаИСебестоимостьПродажОбороты.Ссылка.ЦенаВключаетНДС
	|			ТОГДА ""1""
	|		ИНАЧЕ ""0""
	|	КОНЕЦ,
	|	ВыручкаИСебестоимостьПродажОбороты.ЗаказКлиента.Номер,
	...
	|	ВозвратТоваровОтКлиентаТовары.ДокументРеализации.ЗаказКлиента.Дата,
	|	МАКСИМУМ(ВозвратТоваровОтКлиентаТовары.ДокументРеализации.ЗаказКлиента.Менеджер)
	|ИЗ
	...
	|	ВозвратТоваровОтКлиентаТовары.Ссылка.ХозяйственнаяОперация В (&МассивХозяйственныхОпераций)
	|	И ВозвратТоваровОтКлиентаТовары.Номенклатура.Производитель = &Производитель
	|	И НЕ ВозвратТоваровОтКлиентаТовары.ДокументРеализации.ЗаказКлиента.Номер ЕСТЬ NULL
	|	И ВозвратТоваровОтКлиентаТовары.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	|СГРУППИРОВАТЬ ПО
	|	ВозвратТоваровОтКлиентаТовары.Ссылка.ХозяйственнаяОперация,
	|	ВозвратТоваровОтКлиентаТовары.Ссылка,
	...
	|	И СписаниеНедостачТоваровТовары.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	|СГРУППИРОВАТЬ ПО
	|	ВЫБОР
	|		КОГДА СписаниеНедостачТоваровТовары.Ссылка.Проведен
	|			ТОГДА ""2""
	...
	Пока ВыборкаХозяйственнаяОперация.Следующий() Цикл
		...
		Пока ВыборкаДокументПродажи.Следующий() Цикл
			...
			НомерЗаказа = СформироватьНомерЗаказа(ВыборкаДокументПродажи.ЗаказКлиентаНомер,
				ВыборкаДокументПродажи.ЗаказКлиентаДата);
			ПартнерВнешнийКод = Формат(ВыборкаДокументПродажи.ПартнерВнешнийКод, "ЧГ=");	
			
			Если НЕ ЗначениеЗаполнено(НомерЗаказа) Тогда
				НомерЗаказа = "0";
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(ПартнерВнешнийКод) Тогда
				ПартнерВнешнийКод = "0";
			КонецЕсли;	
				
			ВнешнийКодМенеджера = КодыМенеджеровСоответсвие.Получить(ВыборкаДокументПродажи.Менеджер);
			Если Не ЗначениеЗаполнено(ВнешнийКодМенеджера) Тогда
				ВнешнийКодМенеджера = КодыМенеджеровСоответсвие.Получить(Настройки.ПользовательПоУмолчанию);
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ВнешнийКодМенеджера) Тогда
			...
			ЗаписатьРеквизитXML(ЗаписьXML, "INVOICE_NO", НомерДокумента);
			ЗаписатьРеквизитXML(ЗаписьXML, "OL_CODE", ПартнерВнешнийКод);
			ЗаписатьРеквизитXML(ЗаписьXML, "MERCH_ID", "0");
			ЗаписатьРеквизитXML(ЗаписьXML, "MERCH_CODE", ВнешнийКодМенеджера);
			ЗаписатьРеквизитXML(ЗаписьXML, "DATE", XMLСтрока(ВыборкаДокументПродажи.ДокументПродажиДата));
			ЗаписатьРеквизитXML(ЗаписьXML, "ORDER_NO", НомерЗаказа); // TODO: Идентификатор заказа если Списание ??????
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				ЗаписатьРеквизитXML(ЗаписьXML, "PRICE", Формат(ВыборкаДетальныеЗаписи.Сумма
					/ ВыборкаДетальныеЗаписи.Количество, "ЧЦ=15; ЧДЦ=8; ЧРД=.; ЧН=0; ЧГ=;"));
				ЗаписатьРеквизитXML(ЗаписьXML, "ORDER_NO", НомерЗаказа); // TODO: Идентификатор заказа
				ЗаписатьРеквизитXML(ЗаписьXML, "QTY", Формат(ВыборкаДетальныеЗаписи.Количество,
					"ЧЦ=14; ЧДЦ=3; ЧРД=.; ЧГ=;"));
				ЗаписатьРеквизитXML(ЗаписьXML, "VAT", XMLСтрока(ВыборкаДетальныеЗаписи.СтавкаНДСЧислом));
...
//	*ПользовательПоУмолчанию - СправочникСсылка.Пользователи 
...
Функция ПолучитьНастройки() Экспорт
	...
	ТаблицаКонтрагенты.Колонки.Добавить("Контрагент", Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
	...
	ТаблицаПартнеры.Колонки.Добавить("Партнер", Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип(
		"СправочникСсылка.Партнеры"))));
	ТаблицаПартнеры.Колонки.Добавить("ВнешнийКодПартнер", ОбщегоНазначения.ОписаниеТипаСтрока(36));
	ТаблицаПартнеры.Колонки.Добавить("Контрагент", Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
		Тип("СправочникСсылка.Контрагенты"))));
	...
	ТаблицаНоменклатура.Колонки.Добавить("Номенклатура",
		Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("СправочникСсылка.Номенклатура"))));
	ТаблицаНоменклатура.Колонки.Добавить("ВнешнийКодНоменклатура", ОбщегоНазначения.ОписаниеТипаСтрока(36));

	СлужебныеДанные.Вставить("ТаблицаКонтрагенты", ТаблицаКонтрагенты);
	СлужебныеДанные.Вставить("ТаблицаПартнеры", ТаблицаПартнеры);
	СлужебныеДанные.Вставить("ТаблицаНоменклатура", ТаблицаНоменклатура);
...
Функция ОтправитьФайлНаFTP(Настройки, ПутьКФайлу, ПутьНаСервере = "") Экспорт
	...
	Попытка
		...
		СоединениеFTP = Новый FTPСоединение(Настройки.FTPАдресСервера, Настройки.FTPПорт, Настройки.FTPЛогин,
			Настройки.FTPПароль, Неопределено, Истина, Настройки.FTPТаймаут);
        ...
        // Отправляем файл
        СоединениеFTP.УстановитьТекущийКаталог("/Import");
...
Процедура НачальноеЗаполнение() Экспорт
	...
	Для Каждого ТекущийСправочник Из КакиеСправочникиЗаполнять Цикл
		...
		Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя",
			ТекущийСправочник.Имя + "_SalesWorks_CODE");
...
//      * Ключ     - СправочникСсылка.Пользователи  - ссылка на объект,
//      * Значение - Строка - внешний код менеджера.
Функция ПолучитьКодыМенеджеров(Настройки)
	
	СвойствоПользователь = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя",
		"Пользователи_SalesWorks_CODE");
	...
	Запрос.Текст =
		...
		|	ДополнительныеСведения.Значение КАК ПользователиВнешнийКод
		|ИЗ
		|	Справочник.ГруппыПользователей.Состав КАК ГруппыПользователейСостав
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДополнительныеСведения КАК ДополнительныеСведения
		|		ПО ГруппыПользователейСостав.Пользователь = ДополнительныеСведения.Объект
		|		И ДополнительныеСведения.Свойство = &СвойствоПользователь
		|ГДЕ
		|	ГруппыПользователейСостав.Ссылка = &ГруппПредставители"; 
	
	//@skip-check property-return-type
	Запрос.УстановитьПараметр("ГруппПредставители", Настройки.ГруппаПользователей);
	Запрос.УстановитьПараметр("СвойствоПользователь", СвойствоПользователь);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		//@skip-check property-return-type
		//@skip-check invocation-parameter-type-intersect
		Соответсвие.Вставить(ВыборкаДетальныеЗаписи.Пользователь, Формат(ВыборкаДетальныеЗаписи.ПользователиВнешнийКод, "ЧГ=;"));
	КонецЦикла;
...
// Таблица значений все товары хенкель.
// 
// Параметры:
//  Настройки - Структура
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Таблица значений все товары хенкель
//		*Номенклатура - СправочникСсылка.Номенклатура
//		*ВнешнийКодНоменклатура - Строка

Функция ТаблицаЗначенийВсеТоварыХенкель(Настройки)
	...
	Запрос.Текст =
		...
		|	ДополнительныеСведения.Значение КАК НоменклатураВнешнийКод
		|ИЗ
		|	Справочник.Номенклатура КАК Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДополнительныеСведения КАК ДополнительныеСведения
		|		ПО Номенклатура.Ссылка = ДополнительныеСведения.Объект
	...
	Запрос.УстановитьПараметр("СвойствоНоменклатура", Свойство);
	Запрос.УстановитьПараметр("Производитель", Настройки.ПроизводительБренд);
	
	ТаблицаЗначений = Запрос.Выполнить().Выгрузить();
	
	ТаблицаЗначений.Колонки.Добавить("НомерСтроки", ОбщегоНазначения.ОписаниеТипаЧисло(15));
	ТаблицаЗначений.Колонки.Добавить("ВнешнийКодНоменклатура", ОбщегоНазначения.ОписаниеТипаСтрока(10));
	...
	Для Каждого ТекущаяСтрока ИЗ ТаблицаЗначений Цикл
		...
		//@skip-check property-return-type
		//@skip-check invocation-parameter-type-intersect
		ТекущаяСтрока.ВнешнийКодНоменклатура = Формат(ТекущаяСтрока.НоменклатураВнешнийКод, "ЧГ=");
...
// Параметры:
//  Настройки - Структура
//  УровеньЗаписи - УровеньЖурналаРегистрации
//  Данные - ЛюбаяСсылка
//  Коммнентарий - Строка
Процедура ДобавитьЗаписьВЖурналРегистрации(Настройки, УровеньЗаписи, Данные, Коммнентарий)
	
	Если Настройки.Свойство("РегистрацияСобытий") И Не Настройки.РегистрацияСобытий Тогда
		Возврат;
	КонецЕсли;

	ЗаписьЖурналаРегистрации("SalesWorks", УровеньЗаписи, Неопределено, Данные, Коммнентарий, Неопределено);
