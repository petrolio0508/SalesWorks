	...
	Пока ВыборкаХозяйственнаяОперация.Следующий() Цикл
		...
		Пока ВыборкаДокументПродажи.Следующий() Цикл
			...
			НомерДокумента = СтрШаблон(ШаблонНомераДокумента, СоответсвиеТиповДокументов.Получить(
				...
				"ДФ=yyyy;"), ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ВыборкаДокументПродажи.ДокументПродажиНомер));
	...
	Пока ВыборкаГруппировкаПартнер.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			НомерДокумента = СтрШаблон(ШаблонНомераДокумента, "2", Формат(Выборка.РасчетныйДокументДата, "ДФ=yyyy;"),
				ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.РасчетныйДокументНомер));
...
Процедура Orders(Настройки)
	...
	Пока ВыборкаСсылка.Следующий() Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ЗаписатьРеквизитXML(ЗаписьXML, "LOCALCODE", ВнешнийКодНоменклатура);
			ЗаписатьРеквизитXML(ЗаписьXML, "PRICE", Формат(ВыборкаДетальныеЗаписи.Сумма / ВыборкаДетальныеЗаписи.Количество, "ЧРД=.; ЧГ="));
			ЗаписатьРеквизитXML(ЗаписьXML, "QTY", Формат(ВыборкаДетальныеЗаписи.Количество, "ЧРД=.; ЧГ="));
			ЗаписатьРеквизитXML(ЗаписьXML, "VAT", XMLСтрока(ВыборкаДетальныеЗаписи.СтавкаНДС));
...
Функция СформироватьНомерЗаказа(НомерЗаказа, ДатаЗаказа)
	...
	ШаблонТекста = "%1_%2";
	НомерБезПрефиксов = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(НомерЗаказа);
