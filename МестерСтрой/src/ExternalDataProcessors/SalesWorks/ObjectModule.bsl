...
Процедура ParentCompanies(Настройки) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтрокиТелефона.Количество() > 0 Тогда
			Если ЗначениеЗаполнено(СтрокиТелефона[0].НомерТелефона)
			И СтрДлина(СтрокиТелефона[0].НомерТелефона) <= 20 Тогда
...
Процедура OutletDebts(Настройки) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		НомерДокумента =
				...
				"2",
				Формат(Выборка.РасчетныйДокументДата, "ДФ=yyyy;"), Выборка.РасчетныйДокументНомер);
...
Процедура Orders(Настройки)
	...
	Пока ВыборкаСсылка.Следующий() Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ВнешнийКодНоменклатура = Настройки.СлужебныеДанные.ТаблицаНоменклатура.Найти(ВыборкаДетальныеЗаписи.Номенклатура, "Номенклатура").ВнешнийКодНоменклатура;
...
Процедура SalOuts(Настройки)
	...
	Пока ВыборкаХозяйственнаяОперация.Следующий() Цикл
		...
		Пока ВыборкаДокументПродажи.Следующий() Цикл
			...
			НомерЗаказа = 
				СформироватьНомерЗаказа(ВыборкаДокументПродажи.ЗаказКлиентаНомер, ВыборкаДокументПродажи.ЗаказКлиентаДата);	
...
Функция ПолучитьНастройки() Экспорт
	...
	ТаблицаНоменклатура.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("СправочникСсылка.Номенклатура"))));
